...
public class LibVlcUtil {
    ...
    public static boolean hasCompatibleCPU(Context context) {
        ...
        ElfData elf = readLib(new StringBuilder(String.valueOf(context.getApplicationInfo().dataDir)).append("/lib/libvlcjni.so").toString());
        if (elf == null) {
        ...
        Log.i(str, stringBuilder.append(str2).toString());
        ...
        try {
            ...
            if (elf.e_machine == 3 && !hasX86) {
                ...
                isCompatible = false;
                return false;
            } else if (elf.e_machine == 8 && !hasMips) {
