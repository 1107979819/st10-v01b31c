...
public class Amba2 extends DM368 {
    ...
    public void formatSDCard(Messenger relyTo) {
        ...
        postRequest(this.SERVER_URL + "cgi-bin/cgi?CMD=FORMAT_CARD", relyTo, 14, IPCameraManager.APACHE_GET);
    }
    ...
    public void resetStatus(Messenger relyTo) {
        Log.i(TAG, "reset Status");
        postRequest(this.SERVER_URL + "cgi-bin/cgi?CMD=RESET_STATUS", relyTo, 63, IPCameraManager.APACHE_GET);
    }

    public void requestBind(Messenger relyTo, String macAddress) {
        Log.i(TAG, "reset Status");
        postRequest(this.SERVER_URL + "cgi-bin/cgi?CMD=request_bind&client_mac_address=" + macAddress, relyTo, 1001, IPCameraManager.APACHE_GET);
    }

    public void getBindState(Messenger relyTo) {
        Log.i(TAG, "reset Status");
        postRequest(this.SERVER_URL + "cgi-bin/cgi?CMD=get_bind_state", relyTo, 1002, IPCameraManager.APACHE_GET);
    }

    public void setRtspResolution(Messenger relyTo, String resolution) {
        Log.i(TAG, "reset Status");
        postRequest(this.SERVER_URL + "cgi-bin/cgi?CMD=SET_RTSP_VID&Reslution=" + resolution, relyTo, 1002, IPCameraManager.APACHE_GET);
    }

    protected Object onHandleSpecialRequest(Message msg, Object new_rsp, RequestResult rr) {
        BindStateResponse bindStateResponse;
        Object rtn = new_rsp;
        ...
        switch (msg.arg1) {
            ...
            case IPCameraManager.REQUEST_RESET_STATUS /*63*/:
                ...
                if (IPCameraManager.HTTP_RESPONSE_CODE_OK.equals(rr.result)) {
                    try {
                        ...
                        if (jSONObject.getInt("rval") == 0) {
                            cp = new CameraParams();
                            cp.response = IPCameraManager.HTTP_RESPONSE_CODE_OK;
                            cp.fw_ver = jSONObject.getString("fw_ver");
                            cp.cam_mode = jSONObject.getInt("cam_mode");
                            cp.status = jSONObject.getString("status");
                            cp.iq_type = jSONObject.getInt("iq_type");
                            cp.white_balance = jSONObject.getInt("white_balance");
                            cp.sdFree = jSONObject.getInt("sdfree");
                            cp.sdTotal = jSONObject.getInt("sdtotal");
                            cp.exposure_value = jSONObject.getString("exposure_value");
                            cp.video_mode = jSONObject.getString("video_mode");
                            cp.record_time = jSONObject.getInt("record_time");
                            cp.ae_enable = jSONObject.getInt("ae_enable");
                            cp.audio_sw = jSONObject.getInt("audio_sw");
                            cp.iso = jSONObject.getString("iso_value");
                            cp.shutter_time = jSONObject.getInt("shutter_time");
                            cp.photo_format = jSONObject.getString("photo_format");
                            if (jSONObject.has("audio_enable")) {
                                cp.audio_enable = jSONObject.getInt("audio_enable");
                            } else {
                                cp.audio_enable = 1;
                            }
                            new_rsp = cp;
            ...
            case 1001:
                ...
                if (!IPCameraManager.HTTP_RESPONSE_CODE_OK.equals(rr.result)) {
                    bindResponse = new BindResponse(false, false, null);
                    break;
                }
                try {
                    jSONObject = new JSONObject((String) new_rsp);
                    bindResponse = new BindResponse(true, "ok".equals(jSONObject.getString("result")), jSONObject.getString("server_mac_address"));
                } catch (JSONException e27) {
                    bindResponse = new BindResponse(true, false, null);
                }
                BindResponse rtn3 = new_rsp;
                break;
            case 1002:
                if (!IPCameraManager.HTTP_RESPONSE_CODE_OK.equals(rr.result)) {
                    bindStateResponse = new BindStateResponse(false, true, "ff:ff:ff:ff:ff:ff");
                    break;
                }
                try {
                    jSONObject = new JSONObject((String) new_rsp);
                    bindStateResponse = new BindStateResponse(true, "yes".equals(jSONObject.getString("isbinded")), jSONObject.getString("binded_client_address"));
                } catch (JSONException e28) {
                    bindStateResponse = new BindStateResponse(false, true, "ff:ff:ff:ff:ff:ff");
                }
                BindStateResponse rtn4 = new_rsp;
                break;
        }
